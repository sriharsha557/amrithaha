...
Expected: seven `PASS:` notices and no exception. **Any `FAIL:` means stop and fix the policies before continuing** — every later task assumes this gate is green.
...
Run `sql/verify_rls.sql`. All seven assertions must pass.
...
Then run `sql/verify_rls.sql` in Supabase. Expected: seven `PASS:` notices, no exception.
